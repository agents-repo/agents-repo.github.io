...
Explore app feature ideas with ask-first intake, optional landscape research, options analysis, and a structured feature brief. Chat-web for early brainstorming. Planning only.

- Package: `maiconfz/feature-exploration-planner`
- Version: `1.0.0`
- HTML page: https://agents-repo.org/packages/maiconfz/feature-exploration-planner/
- Markdown: https://agents-repo.org/packages/maiconfz/feature-exploration-planner.md

## Agents

- **automatic-landscape-researcher**: Assumption-first landscape research: proactive search for patterns and competitors, cite sources, label gaps in assumption-log. One-shot.
- **feature-definition-planner**: Synthesize a feature-brief text output from intake, landscape, and options artifacts. Preserves original user intent. One-shot; planning only.
- **feature-exploration-chat**: Early brainstorming for app feature ideas from conversation, public URLs, uploads, or pasted sources. Emits idea-brief when evidence exists. Does not produce a full feature-brief or inspect a host tree.
- **feature-intake**: Ask-first framing of app feature ideas against the host repository. Emits intake-summary and blocking questions only. Does not research or define the full feature brief.
- **interactive-landscape-researcher**: Ask-first landscape research for app features: propose queries, get approval, cite sources. Assumption-log always empty. Invoked once by the flow (not looped); includes one inline query-approval step.
- **options-analyst**: Compare feature implementation options, trade-offs, risks, and non-goals against intake and landscape context. One-shot; no Q&A loop.

## Flows

- **feature-exploration-planning**: Orchestrate ask-first feature intake, routed landscape research, options analysis, and feature-brief synthesis. Planning only; does not implement.
