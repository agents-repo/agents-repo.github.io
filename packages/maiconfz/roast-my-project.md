...
Mean comedy roast of a codebase, website, or app, grounded in real findings. IDE specialists for local trees; chat-web roasts public repos, sites, and store listings. Original parody; not affiliated with any chef or show. Replies match the user's language.

- Package: `maiconfz/roast-my-project`
- Version: `1.1.1`
- HTML page: https://agents-repo.org/packages/maiconfz/roast-my-project/
- Markdown: https://agents-repo.org/packages/maiconfz/roast-my-project.md

## Agents

- **architecture-roaster**: Roast architecture and boundaries from the host tree: coupling, layering, ADRs, and dependency direction. Comedy-club voice. Does not implement. Matches the user's language.
- **brigade-chef**: Brigade and technique tropes. Roasts a host tree when readable, otherwise public repo, website, or app URLs, uploads, or paste. Original parody; does not impersonate anyone. Matches the user's language.
- **code-roaster**: Roast production source from the host tree: naming, duplication, and in-file error handling. Does not own test files. Comedy-club voice. Does not implement. Matches the user's language.
- **fiery-head-chef**: Fiery head-chef tropes. Roasts a host tree when readable, otherwise public repo, website, or app URLs, uploads, or paste. Original parody; does not impersonate anyone. Matches the user's language.
- **project-roaster**: Roast overall project shape from the host tree: README, license, scripts, and CI as process. Comedy-club voice. Does not implement. Matches the user's language.
- **roast-chat**: Chat-web roast of public git repos, websites, or apps from URLs, uploads, or paste. Default comedy-club voice. Does not walk a host tree. Matches the user's language.
- **tests-roaster**: Roast tests, coverage, and test theater from the host tree. Missing tests are findings. Comedy-club voice. Does not implement. Matches the user's language.

## Flows

- **full-roast**: Run project, architecture, code, and tests roasters, then one combined plate plus a constructive sting.

## README

# roast-my-project

Mean comedy roast of a codebase, website, or app, grounded in real
findings. IDE specialists for local trees; chat-web roasts public repos,
sites, and store listings. Original parody; not affiliated with any chef
or show. Replies match the user's language.
...
- **Chat-web:** use `roast-chat` (default voice) or a kitchen agent.
  `roast-chat` analyzes **public git-forge URLs**, **websites**, **apps**
  (one store listing or app-site URL is enough), **uploads**, and
  **pasted sources**. It does not walk a host tree. Kitchen agents walk a
  host tree when readable; otherwise they use the same remote evidence as
  `roast-chat`.

What to paste in chat-web:

- **Git repo:** public forge URL (GitHub and similar).
- **Website:** public HTTPS homepage. Extra pages or screenshots help.
- **App:** one public HTTPS URL (Play Store, App Store, another store
  listing, or the app website). Optional: the other store, screenshots,
  or the privacy policy. Do not send installers.
